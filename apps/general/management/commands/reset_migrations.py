...
def _line_contains_dependency_markers(line):
    return "('" in line and "')" in line and "', '" in line and '_' in line and "get_model(" not in line\
        and 'unique_together' not in line
    ...
    with tempfile.NamedTemporaryFile(mode='w', delete=False) as tmp_file:
        with open(filename) as src_file:
            for line in src_file:
                if pattern_compiled.findall(line) and _line_contains_dependency_markers(line):
...
class Command(BaseCommand):
    ...
    def delete_files_app(self, app):
        ...
        migrations_dir = os.path.join(APPS_PREFIX, app, 'migrations')
        if os.path.exists(migrations_dir):
            shutil.rmtree(os.path.join(APPS_PREFIX, app, 'migrations'))
